...
#include <ppu-lv2.h>
#include "payload.h"
#include "payload_475.h"
#include "syscall8.h"

#include "payload_sky_475_bin.h"
#include "umount_475_bin.h"
...
#define SYSCALL_BASE                    0x8000000000363BE0ULL // 4.75 CEX
#define NEW_POKE_SYSCALL                813
#define NEW_POKE_SYSCALL_ADDR           0x80000000002A1480ULL  // Syscall 813

#define PAYLOAD_OFFSET                  0x3d90
#define PERMS_OFFSET                    0x3560

#define PAYLOAD_UMOUNT_OFFSET           (0x3d90+0x400)
#define UMOUNT_SYSCALL_OFFSET           (0x29FB98 + 0x8) // SYSCALL (838) + 8 -> peek(SYSCALL_BASE + 838*8) -> peek(0x363BE0 + 0x1A30)

#define LV2MOUNTADDR_475 0x8000000000479FC0ULL
//0xff0 => 0x116c (458098 - 459204)
#define LV2MOUNTADDR_475_ESIZE 0x118
#define LV2MOUNTADDR_475_CSIZE 0x108
...
static int lv2_unpatch_bdvdemu_475(void);
static int lv2_patch_bdvdemu_475(uint32_t flags);
static int lv2_patch_storage_475(void);
static int lv2_unpatch_storage_475(void);
...
void set_bdvdemu_475(int current_payload)
{
...
static inline void remove_lv2_memcpy()
{
    ...
    /* restore syscall */
    //remove_new_poke();
// Found @ 0x2A1480 This is per FW, syscall_rmdir_desc // SC813
...
void load_payload_475(int mode)
{
...
//Remove Lv2 memory protection, NOT needed for REBUG 4.7x
    ...
    lv2_memcpy(0x8000000000000000ULL + (u64) PAYLOAD_OFFSET,
                   (u64) payload_sky_475_bin,
                   payload_sky_475_bin_size);
    ...
    */
    ...
    _poke32(0x56600, 0x60000000);             //
    PATCH_JUMP(0x56608, 0x566A0);             //
    _poke32(0x5A6E4,  0x60000000);            // fix 80010009 error
    _poke32(0x5A6F8,  0x60000000);            // fix 80010019 error
    _poke(  0x5658C,  0x63FF003D60000000); // fix 8001003D error  "ori     %r31, %r31, 0x3D\n nop\n" done
    ...
    */
    ...
    PATCH_JUMP(0x29765C, (PAYLOAD_OFFSET+0x30)); // patch openhook - done
...
static int lv2_patch_storage_475(void)
{
    ...
    save_lv2_storage_patch= peekq(0x80000000002ECAB0ULL);
    pokeq32(0x80000000002ECAB0ULL, 0x40000000); //FIXED
...
static int lv2_unpatch_storage_475(void)
{
    ...
    pokeq(0x80000000002ECAB0ULL, save_lv2_storage_patch); //FIXED
...
static int lv2_unpatch_bdvdemu_475(void)
{
    ...
    sys8_memcpy((u64) mem, LV2MOUNTADDR_475, 0x10 * 0x118);
    sys8_memcpy((u64) (mem + 0x1200), 0x80000000007EF020ULL , LV2MOUNTADDR_475_CSIZE);

    for(n = 0; n < 0x116c; n+= LV2MOUNTADDR_475_ESIZE)
    {
        if(!memcmp(mem + n, "CELL_FS_UTILITY:HDD1", 21) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            if(!memcmp(mem + n + 0x69, "temp_bdvd", 10))
            {
                sys8_memcpy(LV2MOUNTADDR_475 + n + 0x69, (u64) "dev_bdvd\0", 10);
                flag++;
            }
        }
        ...
        if(!memcmp(mem + n, "CELL_FS_IOS:USB_MASS_STORAGE0", 29) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            ...
            {
                sys8_memcpy(LV2MOUNTADDR_475 + n + 0x69, (u64) (mem + n + 0x79), 11);
                sys8_memset(LV2MOUNTADDR_475 + n + 0x79, 0ULL, 12);
                flag += 10;
            }
        }
        if(!memcmp(mem + n, "CELL_FS_UTILITY:HDD0", 21) && mem[n-9]== 1 && mem[n-13]== 1)
        {
              ...
              && !memcmp(mem + n + 0x79, "esp_bdvd", 9) && peekq(0x80000000007EF000ULL)!=0)
           {
                ...
                sys8_memcpy(LV2MOUNTADDR_475 + (u64) (n - 0x10), (u64) (mem + 0x1200) , (u64) LV2MOUNTADDR_475_CSIZE);
                flag += 10;
           }
        }
    }
...
static int lv2_patch_bdvdemu_475(uint32_t flags)
{
    ...
    for(n = 0; n < 0x116c; n+= LV2MOUNTADDR_475_ESIZE)
    {
            ...
            && !memcmp(mem + n + 0x69, "dev_bdvd", 9) && mem[n-9]== 1 && mem[n-13]== 1)
        {
            ...
            if(usb >= 0)
                sys8_memcpy(LV2MOUNTADDR_475 + n + 0x69, (u64) "temp_bdvd", 10);
        ...
        {
            if(noBDVD) pos = -1;
            sys8_memcpy(LV2MOUNTADDR_475 + n + 0x69, (u64) "dev_bdvd\0\0", 11);
            sys8_memcpy(LV2MOUNTADDR_475 + n + 0x79, (u64) &path_name[128], 11);
    ...
    if(pos > 0 && pos2 > 0)
    {
      ...
      memcpy(mem + 0x1220, mem + pos2 - 0x10, LV2MOUNTADDR_475_CSIZE);
      dat = LV2MOUNTADDR_475 + (u64) (pos2 - 0x10);
      ...
      n = (int) LV2MOUNTADDR_475_CSIZE; // CDATAS
      memcpy(mem + 0x1214, &n, 0x4);

      memcpy(mem + pos2, mem + pos, LV2MOUNTADDR_475_CSIZE - 0x10);
      memcpy(mem + pos2 + 0x69, "dev_bdvd\0\0", 11);
      memcpy(mem + pos2 + 0x79, "esp_bdvd\0\0", 11);
      memset(mem + pos2 + 0xa4, 0, 8);

      sys8_memcpy(0x80000000007EF000ULL , ((u64) mem + 0x1200), LV2MOUNTADDR_475_CSIZE + 0x20);
      sys8_memcpy(LV2MOUNTADDR_475 + (u64) pos2, ((u64) (mem + pos2)), (u64) (LV2MOUNTADDR_475_CSIZE - 0x10));
